...
Detailed setup and controls for E-Ink Mode. For a quick overview, see the [README](../README.md).
...
<p align="center"><img src="img/welcome.png" width="536" alt="Welcome window with four steps: what it does, try it first, optional extras, where to find it"></p>
...
<p align="center">
  <img src="img/menu-focus.png" width="375" alt="Menu while focusing: Focus 1 of 4 · 18:30 left, Stop Focus Session">
...
<p align="center"><img src="img/menu-color.png" width="375" alt="Menu while temporarily in color: Resume grayscale · 4:32 remaining"></p>
...
<p align="center"><img src="img/settings-schedule.png" width="516" alt="Schedule settings with Never, Evening, and Custom options and a 'What happens next' explanation"></p>
...
- **Switching by hand always wins.** If you turn it off during the evening, the schedule leaves it off until its next change, and the menu says so: *Off by hand · resumes 7:00 AM*.
  ...
  <img src="img/menu-off.png" width="382" alt="Menu showing Schedule · Off by hand · resumes 7:00 AM">
...
- The schedule runs while the app is open. Turn on **Open E-Ink Mode at login** so it works every day. With no schedule or session to resume, opening the app leaves the mode off.
...
<p align="center"><img src="img/settings-appearance.png" width="516" alt="Appearance settings: Grayscale, Dim the screen with slider, Auto-hide the Dock, Reduce motion, Reduce transparency"></p>
...
| Grayscale was already on before you started | It stays on. Restoration uses the settings captured before the session. |
...
E-Ink Mode has no accounts, analytics, or tracking. Update checks contact GitHub; installing an update downloads the release archive. You can turn it off in Settings. Your settings stay on your Mac in `~/Library/Application Support/E-Ink Mode/`.
...
**Is this the same as the grayscale color filter in Accessibility settings?** It uses the same macOS grayscale, plus one-click switching, temporary color, schedules, optional extras, and saved-state restoration.
...
**Does it process my screen?** No. It changes macOS settings rather than capturing or rendering your screen.
...
```sh
./scripts/test.sh     # core and process-level CLI tests against a simulated system
./scripts/build.sh    # universal app + dist/E-Ink-Mode-<VERSION>.zip + install.sh
```

The version comes from [`VERSION`](../VERSION). Set `EINK_SIGN_IDENTITY` (Developer ID Application) and `EINK_NOTARY_PROFILE` (a `notarytool` keychain profile) to produce a signed, notarized build. See [docs/RELEASING.md](RELEASING.md).
